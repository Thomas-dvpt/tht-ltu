    ...
    * Description de la fonction :
    ...
    try {
        ...
        if ('dropdown' in thisItem) {
            ...
            const thisPopup = UI.SysFct.OpenScreenInPopup(`SubNavigation`,screenName,false,``,200, 200 ,false,undefined);
            for (let i = 0; i < maxBP; i++) {
                
              const thisBP = UI.ActiveScreen.FindItem(`~/${screenName}/${prefixBP}${i}`);
              if (! thisBP){
                throw new Error(`l'objet ${screenPath}${prefixBP} est introuvable. Vérifier le le chemin dans le script`);
              }
